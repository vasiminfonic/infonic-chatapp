let express,mongoose,cors,socket,fs,path,nodemailer,PORT,MONGODB_URL,SERVER_Path,errorHandler,router,getCurrentUser,userDisconnect,joinUser,getCurrentRoom,checkRoom,totalOnlineUsers,Message;_75f‍.w("express",[["default",["express"],function(v){express=v}]]);_75f‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_75f‍.w("cors",[["default",["cors"],function(v){cors=v}]]);_75f‍.w("socket.io",[["default",["socket"],function(v){socket=v}]]);_75f‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_75f‍.w("path",[["default",["path"],function(v){path=v}]]);_75f‍.w("nodemailer",[["default",["nodemailer"],function(v){nodemailer=v}]]);_75f‍.w("./config",[["PORT",["PORT"],function(v){PORT=v}],["MONGODB_URL",["MONGODB_URL"],function(v){MONGODB_URL=v}],["SERVER_Path",["SERVER_Path"],function(v){SERVER_Path=v}]]);_75f‍.w("./errorHandler",[["errorHandler",["errorHandler"],function(v){errorHandler=v}]]);_75f‍.w("./routers",[["default",["router"],function(v){router=v}]]);_75f‍.w("./socketUsers",[["getCurrentUser",["getCurrentUser"],function(v){getCurrentUser=v}],["userDisconnect",["userDisconnect"],function(v){userDisconnect=v}],["joinUser",["joinUser"],function(v){joinUser=v}],["getCurrentRoom",["getCurrentRoom"],function(v){getCurrentRoom=v}],["checkRoom",["checkRoom"],function(v){checkRoom=v}],["totalOnlineUsers",["totalOnlineUsers"],function(v){totalOnlineUsers=v}]]);_75f‍.w("./models/message",[["default",["Message"],function(v){Message=v}]]);
...
db.on("error", _75f‍.g.console.error.bind(_75f‍.g.console, "connection error:"));
db.once("open", () => {
...
io.on("connection", (socket) => {
  ...
  socket.on("joinChat", ({ sender, _id }) => {
    ...
    _75f‍.g.console.log(socket.id, "=id");
    const isInRoom = socket.rooms.has(_id);

    _75f‍.g.console.log(isInRoom, "this is rooms");
    socket.join(_id);
  ...
  socket.on("chat", async (value) => {
    ...
    if (value._id != undefined) {
      ...
      _75f‍.g.console.log(value.text, 'socketMessge')
      if (value.orderId) {
      ...
      _75f‍.g.console.log(empty, "client number of room");
  ...
  socket.on("disconnect", () => {
    ...
    _75f‍.g.console.log(dUser, "disconnected");
    if (dUser) {
