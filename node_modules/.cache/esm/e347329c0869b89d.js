let express,mongoose,cors,socket,fs,path,PORT,MONGODB_URL,SERVER_Path,errorHandler,router,getCurrentUser,userDisconnect,joinUser,getCurrentRoom,checkRoom,totalOnlineUsers,Message;_086‍.w("express",[["default",["express"],function(v){express=v}]]);_086‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_086‍.w("cors",[["default",["cors"],function(v){cors=v}]]);_086‍.w("socket.io",[["default",["socket"],function(v){socket=v}]]);_086‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_086‍.w("path",[["default",["path"],function(v){path=v}]]);_086‍.w("./config/index.js",[["PORT",["PORT"],function(v){PORT=v}],["MONGODB_URL",["MONGODB_URL"],function(v){MONGODB_URL=v}],["SERVER_Path",["SERVER_Path"],function(v){SERVER_Path=v}]]);_086‍.w("./errorHandler",[["errorHandler",["errorHandler"],function(v){errorHandler=v}]]);_086‍.w("./routers",[["default",["router"],function(v){router=v}]]);_086‍.w("./socketUsers",[["getCurrentUser",["getCurrentUser"],function(v){getCurrentUser=v}],["userDisconnect",["userDisconnect"],function(v){userDisconnect=v}],["joinUser",["joinUser"],function(v){joinUser=v}],["getCurrentRoom",["getCurrentRoom"],function(v){getCurrentRoom=v}],["checkRoom",["checkRoom"],function(v){checkRoom=v}],["totalOnlineUsers",["totalOnlineUsers"],function(v){totalOnlineUsers=v}]]);_086‍.w("./models/message",[["default",["Message"],function(v){Message=v}]]);
...
db.on("error", _086‍.g.console.error.bind(_086‍.g.console, "connection error:"));
db.once("open", () => {
...
io.on("connection", (socket) => {
  ...
  socket.on("joinChat", ({ sender, _id }) => {
    ...
    _086‍.g.console.log(socket.id, "=id");
    const isInRoom = socket.rooms.has(_id);

    _086‍.g.console.log(isInRoom,`this is rooms: ${_id}`);
    socket.join(_id);
  ...
  socket.on("chat", async (value) => {
    ...
    if (value._id != undefined) {
      ...
      _086‍.g.console.log(value.text, 'socketMessge')
      if (value.orderId) {
      ...
      if(clientsInRoom){
        if(clientsInRoom.size){
          _086‍.g.console.log(clientsInRoom.size, `${value.receiver} room size is ${clientsInRoom.size}`)
        }
      }else{
        console.log(`${value.receiver} room is empty`)
      }
      _086‍.g.console.log(clientsInRoom,'ClientInRoom')
  ...
  socket.on("disconnect", () => {
    ...
    _086‍.g.console.log(dUser, "disconnected");
    if (dUser) {
